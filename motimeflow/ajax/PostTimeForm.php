...
foreach($_POST["omschrijving"] as $value) {
    if (empty($value)) {
        array_push($errors, "Controleer of je bij alle rijen een omschrijving hebt ingevuld.");
        break;
    }
}
...
if (count($errors) > 0) {
    ...
    foreach($_POST["tijd"] as $value) {
        ...
        $query = "INSERT INTO moTimeflow_tijden_v2 (`klant_id`, `pauzeTijd`, `weekDagen`, `pulsDuur`, `omschrijving`, `rooster`, `relayID`, `relayNummer`, `aanuit`)
        VALUES (:klant_nmr, :tijd, :dagenString, :pulsduur, :omschrijving, :rooster, :relay_id, :relaynummer, :aanuit)";
        ...
        $stmt->bindValue(':rooster', $_SESSION["current"]["rooster"]);
        $stmt->bindValue(':relay_id', $_SESSION["user"]["relay_id"]);
    ...
    exit('<p class="text-success my-1"><i class="fa-fw fa-solid fa-check me-1"></i>De nieuwe data is succesvol toegevoegd!</p>');     
}
